 ...
 */
class getSecurity_Firewall_Map_Context_AuthClientService extends App_KernelDevDebugContainer
{
    /**
     * Gets the private 'security.firewall.map.context.authClient' shared service.
     *
    ...
    public static function do($container, $lazyLoad = true)
    {
        ...
        $c->addListener('Symfony\\Component\\Security\\Http\\Event\\LogoutEvent', [0 => function () use ($container) {
            return ($container->privates['security.logout.listener.default.authClient'] ?? $container->load('getSecurity_Logout_Listener_Default_AuthClientService'));
        }, 1 => 'onLogout'], 64);
        $c->addListener('Symfony\\Component\\Security\\Http\\Event\\LogoutEvent', [0 => function () use ($container) {
            return ($container->privates['security.logout.listener.session.authClient'] ?? ($container->privates['security.logout.listener.session.authClient'] = new \Symfony\Component\Security\Http\EventListener\SessionLogoutListener()));
        }, 1 => 'onLogout'], 0);
        $c->addListener('Symfony\\Component\\Security\\Http\\Event\\LogoutEvent', [0 => function () use ($container) {
        ...
        return $container->privates['security.firewall.map.context.authClient'] = new \Symfony\Bundle\SecurityBundle\Security\LazyFirewallContext(new RewindableGenerator(function () use ($container) {
            ...
            yield 2 => ($container->privates['security.authentication.listener.guard.authClient'] ?? $container->load('getSecurity_Authentication_Listener_Guard_AuthClientService'));
            yield 3 => ($container->privates['security.authentication.listener.anonymous.authClient'] ?? $container->load('getSecurity_Authentication_Listener_Anonymous_AuthClientService'));
            yield 4 => ($container->privates['security.access_listener'] ?? $container->load('getSecurity_AccessListenerService'));
        }, 5), new \Symfony\Component\Security\Http\Firewall\ExceptionListener($a, ($container->privates['security.authentication.trust_resolver'] ?? ($container->privates['security.authentication.trust_resolver'] = new \Symfony\Component\Security\Core\Authentication\AuthenticationTrustResolver())), $b, 'authClient', ($container->privates['App\\Security\\ConnecterAuthenticator'] ?? $container->load('getConnecterAuthenticatorService')), NULL, NULL, ($container->privates['monolog.logger.security'] ?? $container->load('getMonolog_Logger_SecurityService')), false), new \Symfony\Component\Security\Http\Firewall\LogoutListener($a, $b, $c, ['csrf_parameter' => '_csrf_token', 'csrf_token_id' => 'logout', 'logout_path' => 'app_logout']), new \Symfony\Bundle\SecurityBundle\Security\FirewallConfig('authClient', 'security.user_checker', NULL, true, false, 'security.user.provider.concrete.app_client_provider', 'authClient', 'App\\Security\\ConnecterAuthenticator', NULL, NULL, [0 => 'guard', 1 => 'anonymous'], NULL), ($container->privates['security.untracked_token_storage'] ?? ($container->privates['security.untracked_token_storage'] = new \Symfony\Component\Security\Core\Authentication\Token\Storage\TokenStorage())));
    }
}
